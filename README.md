...
Jump to any slide directly via the URL hash, e.g. `#35` for Assignment 1.
...
- (`timer` is no longer used: assignment timers have no preset, you set the
  minutes on the slide.)
...
| `slides.js` | All 91 slides as one JSON-shaped data file — the only file most edits touch |
...
- **One flat 91-slide array**, not a multi-squad/multi-day picker — this
  deck is always exactly these 91 slides in this order, so the "Choose
...
- **Slide type is set explicitly** (a `type` field on every slide) rather
  ...
  With 91 hand-authored slides, explicit typing is more reliable than
...
1. **Click through all 91 slides in order**, start to finish, both days.
